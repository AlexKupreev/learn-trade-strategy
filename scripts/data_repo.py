...
class DataRepository:
    ...
    min_date: str
    ALL_TICKERS: list[str] = US_STOCKS + EU_STOCKS + INDIA_STOCKS + CHINA_STOCKS
    ...
    def fetch_tickers(self, min_date=None):
        ...
        print(f'Going download data for these tickers: {self.ALL_TICKERS}')
        ...
        for i, ticker in enumerate(tq):
            ...
            # historical returns
            for i in [1, 3, 7, 30, 90, 365]:
                historyPrices['growth_' + str(i) + 'd'] = historyPrices['Adj Close'] / historyPrices['Adj Close'].shift(
                    i)
