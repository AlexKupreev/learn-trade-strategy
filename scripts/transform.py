...
class TransformData:
    ...
    def _transform_tickers(self):
        ...
        for ticker in tickers:
            ...
            df_current_ticker_volume_indicators['Date'] = pd.to_datetime(df_current_ticker_volume_indicators['Date'],
                                                                         utc=True)
            df_current_ticker_pattern_indicators['Date'] = pd.to_datetime(df_current_ticker_pattern_indicators['Date'],
                                                                          utc=True)

            # merge to one df
            m1 = pd.merge(current_ticker_df, df_current_ticker_momentum_indicators.reset_index(), how='left',
