...
class TrainModel:
    ...
    def _define_dummies(self):
        ...
        self.transformed_df["wom"] = self.transformed_df["Date"].apply(
            lambda d: (d.day - 1) // 7 + 1
        )
        self.transformed_df.loc[:, "wom"] = self.transformed_df.loc[:, "wom"].astype(
            str
        )

        self.transformed_df.loc[:, "month_wom"] = (
            self.transformed_df.Month + "_w" + self.transformed_df.wom
        )
    ...
    def make_inference(self, sim_params: SimulationParams):
        ...
        self.df_full[self.PREDICTION_NAME] = (
            y_pred_all_class1_array >= sim_params.threshold
        ).astype(int)
        # define rank of the prediction - only for predicted stocks
        self.df_full[f"{self.PREDICTION_NAME}_rank"] = self.df_full.groupby("Date")[
    ...
    def rolling_max_min(df: pd.DataFrame, window=5):
        ...
        # high/low in 5 days
        df["Max_High_Next_5"] = (
            df["High"].rolling(window=window, min_periods=1).max().shift(-window + 1)
        )
        df["Min_Low_Next_5"] = (
            df["Low"].rolling(window=window, min_periods=1).min().shift(-window + 1)
        )

        # low in 1 day (for lower entry)
        df["Min_Low_Next_1"] = (
            df["Low"].rolling(window=1, min_periods=1).min().shift(-1)
        )
        return df
    ...
    def extend_df_for_simulation(self):
        ...
        result = (
            self.df_full[["Date", "High", "Low", "Adj Close", "Ticker"]]
            .groupby("Ticker")
            .apply(self.rolling_max_min)
            .reset_index(drop=True)
        )

        # Calculate the ratios + safe divide
        result["Ratio_MaxHighNext5_to_AdjClose"] = np.where(
            result["Adj Close"] == 0,
            np.nan,
            result["Max_High_Next_5"] / result["Adj Close"],
        )
        result["Ratio_MinLowNext5_to_AdjClose"] = np.where(
            result["Adj Close"] == 0,
            np.nan,
            result["Min_Low_Next_5"] / result["Adj Close"],
        )

        result["Ratio_MinLowNext1_to_AdjClose"] = np.where(
            result["Adj Close"] == 0,
            np.nan,
        ...
        self.df_full = self.df_full.merge(
            result[
                [
                    ...
                    "Max_High_Next_5",
                    "Min_Low_Next_5",
                    "Ratio_MaxHighNext5_to_AdjClose",
                    "Ratio_MinLowNext5_to_AdjClose",
    ...
    def get_future_gross_return(row, sim_params: SimulationParams):
        ...
        if row["take_profit"] == 1 and row["stop_loss"] == 1:
            if random.random() > 0.5:  # assume take_profit event was first
                return row["investment"] * (
                    sim_params.take_profit + (1 - sim_params.lower_entry)
                )
            else:  # assume stop_loss event was first
                return row["investment"] * (
                    sim_params.stop_loss + (1 - sim_params.lower_entry)
                )
        ...
        if pd.isna(row["growth_future_5d"]):
            return row[
                "investment"
            ]  # no information on growth in 5 days --> return the same investment in 5 days
        else:
            return row["investment"] * (
                row["growth_future_5d"] + (1 - sim_params.lower_entry)
            )
    ...
    def one_date_simulation(
        ...
        result_df["lower_entry"] = (
            result_df["Ratio_MinLowNext1_to_AdjClose"] <= sim_params.lower_entry
        ).astype(int)
        ...
        result_df["stop_loss"] = (
            result_df["Ratio_MinLowNext5_to_AdjClose"] <= sim_params.stop_loss
        ).astype(int)

        # 7. Take Profit: take the money if the current Price (or Max_price) goes higher than sim_params.take_profit
        result_df["take_profit"] = (
            result_df["Ratio_MaxHighNext5_to_AdjClose"] >= sim_params.take_profit
        ).astype(int)
        ...
        result_df["fees"] = result_df.apply(
            lambda row: self.get_fees(row, sim_params=sim_params), axis=1
        )
        result_df["future_net_return"] = (
            result_df["future_gross_return"] + result_df["fees"]
        )
    ...
    def simulate(self, params: SimulationParams):
        ...
        all_dates = (
            self.df_full[self.df_full.split == "test"]
            .sort_values(by="Date")
            .Date.unique()
        )
        ...
        for current_date in all_dates[0:-5]:
            ...
            if simulation_df is None:
                ...
                simulation_df = pd.concat(
                    [simulation_df, one_day_simulation_results], ignore_index=True
                )
        ...
        stop_loss_filter = (simulation_df.lower_entry == 1) & (
            simulation_df.stop_loss == 1
        )
        print(
        ...
        take_profit_filter = (simulation_df.lower_entry == 1) & (
            simulation_df.take_profit == 1
        )
        print(
        ...
        print(
            f"  Start capital = {params.initial_capital}, Resulting capital: {capital_df[-5:].capital.sum()} "
        )
        print(
    ...
    def get_last_signals(self, num: int = 10):
        ...
        COLUMNS = [
            ...
            "Date",
            self.PREDICTION_NAME,
            self.PREDICTION_NAME + "_rank",
        ]
        signals = self.df_full[
            (self.df_full[f"{self.PREDICTION_NAME}_rank"] <= MIN_RANK)
            & (self.df_full[self.PREDICTION_NAME] > 0)
        ].sort_values(by=["Date", f"{self.PREDICTION_NAME}_rank"])
